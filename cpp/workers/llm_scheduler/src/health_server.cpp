...
bool HealthServer::live() const {
  return !stopping_.load() && now_ms() - state_.tick_ms.load() < 15000;
}

bool HealthServer::ready() const {
  return live() && state_.postgres.load() && state_.redis.load() && state_.web.load();
}
  ...
  try {
    ...
    while (!stopping_.load()) {
      ...
      http::response<http::string_body> response{
          known ? (healthy ? http::status::ok : http::status::service_unavailable) : http::status::not_found,
          request.version()};
    ...
    std::cerr << "LLM Worker 健康服务退出: " << error.what() << '\n';
  }
}
