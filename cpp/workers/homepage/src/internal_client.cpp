...
int progress_callback(void* state,curl_off_t,curl_off_t,curl_off_t,curl_off_t){ return static_cast<const std::atomic<bool>*>(state)->load()?1:0; }
...
HomePageGenerationResult InternalClient::execute(const RunTask& task) const{
  CurlHandle curl; if(!curl.value) throw WorkerError("GENERATOR_CONNECTION_ERROR","无法初始化 libcurl",true); std::string response;
  ...
  if(code!=CURLE_OK) throw WorkerError(code==CURLE_OPERATION_TIMEDOUT?"GENERATOR_TIMEOUT":"GENERATOR_CONNECTION_ERROR",curl_easy_strerror(code),true);
  if(status==409) throw WorkerError("STALE_GENERATION_TASK",response,false);
  if (status < 200 || status >= 300) {
    throw WorkerError("GENERATOR_HTTP_" + std::to_string(status), response,
