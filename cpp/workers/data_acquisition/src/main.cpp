...
int main() {
  ...
  try {
    ...
    auto transport = std::make_shared<messaging::RedisStreamTransport>(settings(config));
    ...
    auto runtime = task_lifecycle::make_worker<AcquisitionAttemptInput, ProviderFetchResult>(
        std::move(runtime_config), repository,
        [&config](const AcquisitionTask& task, std::stop_token stop_token) {
          return ProviderClient(config).execute(task, stop_token);
        });
    ...
    health.provider.store(ProviderClient(config).health());
    ...
    std::cerr << "data-acquisition-worker 启动失败: " << error.what() << '\n';
    curl_global_cleanup();
    return 1;
  }
  curl_global_cleanup();
