...
describe("workflow diagram specs", () => {
  it("returns the latest quick research spec for static entry previews", () => {
    const spec = getLatestWorkflowDiagramSpec("industry_research");

    expect(spec?.templateCode).toBe("industry_research");
  ...
  it("uses Chinese labels for interactive workflow diagram nodes", () => {
    const quickResearchSpec = getWorkflowDiagramSpec("industry_research", 3);
