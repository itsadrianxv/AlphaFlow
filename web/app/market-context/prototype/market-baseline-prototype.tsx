...
import {
  MARKET_BASELINE_CHARTS,
  MARKET_BASELINE_ITEMS,
...
function MarketCharts({ phase }: { phase: Phase }) {
  const snapshot = MARKET_BASELINE_CHARTS[phase];
  return (
  ...
  return (
    <div className="mb-5 flex flex-wrap items-start justify-between gap-4 border border-[var(--app-warning-border)] bg-[var(--app-warning-surface)] px-4 py-3 text-sm">
      ...
      <div className="app-data shrink-0 text-xs text-[var(--app-text-subtle)]">
        数据截至 {PROFESSIONAL_MARKET_BASELINE.globalActualDataCutoff}
      </div>
    </div>
  );
}
  ...
  return (
    <aside className="border-l border-[var(--app-border-soft)] bg-[var(--app-bg-raised)] p-5 lg:w-[320px] lg:shrink-0">
      ...
      <dl className="mt-5 grid gap-3 border-y border-[var(--app-border-soft)] py-4 text-xs">
        ...
        <div className="flex justify-between gap-3">
          <dt className="text-[var(--app-text-subtle)]">限制</dt>
          <dd className="text-right text-[var(--app-text-muted)]">
            {item.limitation ?? "必需数据已达到本区域目标截止点。"}
          </dd>
        </div>
        <div className="flex justify-between gap-3">
          <dt className="text-[var(--app-text-subtle)]">降级</dt>
          <dd className="text-right text-[var(--app-text-muted)]">
            {item.degradation ?? "正常读取。"}
          </dd>
        </div>
      </dl>
      <div className="mt-5">
...
export function MarketBaselinePrototype() {
  ...
  const phaseCounts = useMemo(
    () =>
      Object.fromEntries(
        PHASES.map((phase) => [
          phase.id,
          MARKET_BASELINE_ITEMS.filter((item) => item.phase === phase.id)
            .length,
        ]),
  ...
  const visibleItems = useMemo(
    () => MARKET_BASELINE_ITEMS.filter((item) => item.phase === activePhase),
    [activePhase],
  );
  ...
  const selectedItem =
    MARKET_BASELINE_ITEMS.find((item) => item.id === selectedId) ?? null;
