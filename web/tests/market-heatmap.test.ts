...
describe("A 股概念热力图", () => {
  it("用绿色表达下跌、粉色表达上涨、灰色表达平盘", () => {
    expect(getHeatmapColor(-3, "dark")).toBe("#2d733d");
    expect(getHeatmapColor(3, "dark")).toBe("#bb4e70");
    expect(getHeatmapColor(0, "dark")).toBe("#3a3d40");
  });
  ...
  it("按概念和市值生成嵌套 Treemap 数据", () => {
    ...
    expect(series.data[0]?.children[0]?.itemStyle.color).toBe("#bb4e70");
    expect(series.data[0]?.children[1]?.itemStyle.color).toBe("#2d733d");
  });
});
