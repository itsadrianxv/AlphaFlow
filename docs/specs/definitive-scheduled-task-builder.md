...
19. 作为核心投资者，我希望规则使用有符号分值变化，以便多头条件加分、空头条件扣分，同时不被系统隐式归一化。
...
- 条件使用受控 JSONLogic：支持 `var`、比较、`and/or/!`、`in` 以及 `cross_above/cross_below`。条件树最多 8 层、200 个节点；超过约 3 层时使用进入条件组的聚焦编辑视图。规则最多 50 条，指标声明最多 20 个。
- JSONLogic 只组合已计算的标准化指标快照，不负责指标计算、取数、周期聚合或数据需求推导。MVP 不支持算术、脚本、数组遍历、任意自定义公式或背离操作符。
...
- 每条规则使用有符号 `scoreDelta`，命中后加到总分；未命中或未评估贡献 0，总分不归一化。结果同时记录理论最低分与理论最高分；`minScore` 高于最高可能得分仍允许保存和启用，但显示“不会产生入选结果”警告。
