...
StreamMessage RedisStream::parse_message(const std::string& message_id, const redisReply& fields) {
  ...
  StreamMessage message{message_id, values["eventId"], values["runId"], values["createdAt"], values["schemaVersion"]};
  if (message.schema_version != "1" || message.event_id.empty() || message.run_id.empty() || message.created_at.empty()) {
...
std::string RedisStream::publish(const StreamMessage& message) {
  auto reply = command("XADD %s * schemaVersion %s eventId %s runId %s createdAt %s", config_.stream.c_str(),
                       message.schema_version.c_str(), message.event_id.c_str(), message.run_id.c_str(),
                       message.created_at.c_str());
  if (reply->type == REDIS_REPLY_ERROR) throw std::runtime_error(reply_string(reply.get()));
