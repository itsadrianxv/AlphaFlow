...
Config Config::from_environment() {
  ...
  config.worker_threads = positive_int("SCREENING_WORKER_THREADS", 4);
  config.queue_capacity = positive_int("SCREENING_WORKER_QUEUE_CAPACITY", 8);
  config.lease_seconds = positive_int("SCREENING_WORKER_LEASE_SECONDS", 90);
  config.heartbeat_seconds = positive_int("SCREENING_WORKER_HEARTBEAT_SECONDS", 30);
  config.claim_idle_ms = positive_int("SCREENING_WORKER_CLAIM_IDLE_MS", 120000);
  config.python_timeout_ms = positive_int("SCREENING_WORKER_PYTHON_TIMEOUT_MS", 900000);
  config.health_port = positive_int("SCREENING_WORKER_HEALTH_PORT", 8030);
  config.stream_block_ms = positive_int("SCREENING_WORKER_STREAM_BLOCK_MS", 5000);
  config.stream = env_or("SCREENING_RUN_STREAM", "screening:runs");
  config.group = env_or("SCREENING_RUN_GROUP", "screening-worker");
  config.worker_id = identity();
  config.consumer = config.worker_id;
  if (config.heartbeat_seconds >= config.lease_seconds) {
    throw std::runtime_error("SCREENING_WORKER_HEARTBEAT_SECONDS must be less than lease seconds");
  }
  if (config.health_port > 65535) throw std::runtime_error("SCREENING_WORKER_HEALTH_PORT must be <= 65535");
  return config;
}
