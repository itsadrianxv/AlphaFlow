...
void WorkerRuntime::handle_message(const StreamMessage& message) {
  try {
    ...
    if (claim.status == ClaimStatus::busy) {
      // 该消息可能就是崩溃 worker 留下的唯一 PEL 记录；保留到 lease 到期后再接管。
      return;
    }
    ...
    std::cerr << "领取确定性评分任务失败 " << message.run_id << ": " << error.what() << '\n';
  }
}
...
void WorkerRuntime::execute(RunTask task) {
  ...
  try {
    ...
    std::cerr << "确定性评分任务失败 " << task.message.run_id << " [" << error.code() << "]: " << error.what() << '\n';
    if (stopping_.load()) {
      remove_lease(task);
      return;
    }
    ...
    std::cerr << "确定性评分任务内部错误 " << task.message.run_id << ": " << error.what() << '\n';
